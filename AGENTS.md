...
port in progress** on branch `multiplatform-port` (PR #1): Windows = shipped
(v0.1.0); macOS (Apple Silicon) + Linux porting — **Phase 1 (cross-platform
build + CI) + Phase 2 (macOS code, Tasks 4-9) COMPLETE & CI-green**; Phase 3
(Linux) next. See "Multi-platform port status" below.
...
- **Resolved (`src-tauri/Cargo.lock`):**
  ...
  - **ASR = TWO crates: `transcribe-rs` 0.3.11 (GigaAM) + `parakeet-rs` 0.3.7 (Nemotron).** `ort` resolves to **2.0.0-rc.13**. ⚠ **ort-pin landmine (CONFIRMED load-bearing 2026-08-07, multi-platform port Phase 1):** `transcribe-rs 0.3.11` pins `ort = "=2.0.0-rc.12"` (EXACT), `parakeet-rs 0.3.7` requires `2.0.0-rc.13` — mutually unsatisfiable. molvi's `[patch.crates-io]` transcribe-rs git override (Cargo.toml, rev `efc66111…`) relaxes the pin. **A clean resolve SUCCEEDS on all 3 OSes** (verified by the windows/macos-14/ubuntu CI matrix). Do NOT remove the patch. ort emits tracing directly → see Logs.
  - **macOS target deps** (`[target.'cfg(target_os="macos")'.dependencies]`): `tauri-nspanel = { git = "https://github.com/ahkohd/tauri-nspanel", branch = "v2.1" }` (NOT on crates.io — verified; overlay focus fix), `objc2 = "0.6"` (resolves 0.6.4), `objc2-foundation = "0.3"` (features NSURL+NSString), `objc2-app-kit = "0.3"` (features **NSWorkspace+NSRunningApplication REQUIRED** — bare "0.3" does NOT enable them; they're per-class features).
  - **Apple-Silicon CoreML engine features** (`[target.'cfg(all(target_os="macos", target_arch="aarch64"))'.dependencies]`): `transcribe-rs` feature `ort-coreml` (= `["onnx","ort/coreml"]`, so it transitively enables onnx+audio-features), `parakeet-rs` `default-features=false` + `["cpu","ort-defaults","api-28","coreml"]`. ort/coreml COMPILES on aarch64-apple-darwin (macos-14 CI green = spike #2 CoreML dimension ✓). Runtime CoreML speedup vs ort-CPU = Task 10 measurement (deferred, human on Mac).
  - serde 1.0.229, serde_json 1.0.151, tracing 0.1.44, tracing-subscriber 0.3.23, tracing-appender 0.2.5
  - **`thiserror`: BOTH 1.0.69 + 2.0.19 present** (a transitive dep still pulls 1.x; molvi's own code uses 2). **`windows`: BOTH 0.61.x + 0.62.2 present; the `windows` crate is now a `[target.'cfg(windows)'.dependencies]` entry (Phase-1 Task 2 moved it from global for cross-platform compile).**
...
- **`profiles.rs`** — per-app post-proc (NO DB). `foreground_exe() -> Result<String>` (cross-platform: Win32 HWND→PID→`QueryFullProcessImageNameW`→basename; macOS `NSWorkspace::sharedWorkspace().frontmostApplication().bundleURL().path()`→basename; Linux stub Err until Phase 3) + `macos_frontmost_pid() -> Option<isize>` (Task 8; filters kNoProcess -1 → None, consumed by the paste focus-guard) + `resolve(profiles, exe) -> Option<&ProfileEntry>` (first enabled case-insensitive match) + `apply_profile_override(&mut PostProcessing, Option<&ProfileEntry>)` (post_mode always overrides; prompt only if Some; endpoint/model/api_key never). All UPPERCASED basename contract so `resolve()` matches unchanged across OSes.
...
- **`paths.rs`** — cross-platform app-data path resolution: Windows `%APPDATA%\com.molvi.app\`, macOS `~/Library/Application Support/com.molvi.app/`, Linux `$XDG_CONFIG_HOME` (default `~/.config`) `\com.molvi.app\`. `redact_appdata(&Path) -> String` for privacy-safe logging (redacts `%APPDATA%` on Windows, `$HOME`→`~` on Unix; stdlib `var_os` + `strip_prefix`, no `dirs` dep).
...
- **Paste + command-chord keys are per-platform (IMPLEMENTED Task 6, cross-platform).** Helpers in `paste.rs`: `paste_modifier() -> enigo::Key` (Windows/Linux = `Key::Control`; macOS = **`Key::Meta`** — there is NO `Key::Command` variant; `Key::Meta` IS Command on macOS) + `paste_key() -> enigo::Key` (Windows = `Key::Other(0x56)` VK_V — enigo's Unicode path is rejected as Ctrl+V by some Windows apps; macOS = **`Key::Other(9)`** virtualKey 9 = physical V, layout-robust under ⌘ per cjpais/Handy + VoiceInk — NOT `Key::Unicode('v')`; Linux/X11 = `Key::Unicode('v')`). `commands.rs::letter_key(char) -> Key` (a `pub(crate) const fn`) resolves command-chord letter keys per platform: Windows = VK (`Key::Other(ascii_upper)`, A=0x41..Z=0x5A); macOS = **`Key::Other(kVK_ANSI_*)`** (A=0x00,C=0x08,V=0x09,X=0x07,Y=0x10,Z=0x06) — NOT `Key::Unicode`, which under a non-QWERTY layout maps to the wrong physical key (AZERTY 'z'→keycode 0x10 → ⌘Y redo instead of ⌘Z undo); Linux = `Key::Unicode(char)` (XKB keysym via layout). The static `PHRASES` table + test helpers call `letter_key` so they're platform-correct at compile time. macOS paste focus-guard is **verify-only** (spike #3): a background/accessory app cannot reliably re-activate another app; if the frontmost pid changed mid-dictation, refuse (leave text on clipboard + toast).
- macOS overlay needs `tauri-nspanel` (Tauri `focusable:false` is broken there — tauri#14102); the `macos_overlay` module (lib.rs) converts the overlay window to a non-activating NSPanel (`can_become_key_window:false`). macOS Secure Input detection (`secure_input_held()`, Carbon `IsSecureEventInputEnabled`) warns when another app holds Secure Input (silently suppresses the global hotkey — Handy #1578).
...
## Multi-platform port status (2026-08-07)

Design spec + 13-task plan + competitor research: `docs/superpowers/{specs,plans}/2026-08-07-molvi-multiplatform-port*`, `docs/superpowers/specs/2026-08-07-paste-focus-guard-spike.md` (spike #3), `docs/spike-results.md` (Phase-1 spike outcomes). **Execution state lives in the SDD ledger** `.superpowers/sdd/2026-08-07-molvi-multiplatform-port/progress.md` (the recovery map — read it FIRST in any session resuming the port). Branch `multiplatform-port` (PR #1, base `main`). CI: `.github/workflows/ci.yml` (windows-latest + macos-14 + ubuntu-latest: fmt + clippy `-D warnings` + `test --lib` + tsc + build).

**Phase 1 COMPLETE & CI-green:** license (MIT OR Apache-2.0), Step-0 cfg-gate (the 5 Win32 sites + `windows`→target-dep + paths tests gated), CI matrix. Spikes PASS: ort/transcribe-rs/parakeet-rs resolve+compile on aarch64-apple-darwin (#2) + x86_64-linux (#1); `[patch]` transcribe-rs override confirmed load-bearing.

**Phase 2 CODE COMPLETE & CI-green (Tasks 4-9):** macOS Cargo deps (CoreML engines + tauri-nspanel + objc2); paths cross-platform; NSWorkspace foreground_exe + pid helper; per-platform paste keys (⌘V) + verify-only focus guard + command-chord `letter_key`; NSPanel overlay (tauri-nspanel); Info.plist + Secure Input detection. All compile-verified on macos-14 CI.
...
2. **objc2-app-kit needs per-class features** `["NSWorkspace","NSRunningApplication"]` (bare `"0.3"` insufficient; tauri-nspanel enables different features transitively).
3. **macOS command chords use `kVK_ANSI` VKs**, NOT `Key::Unicode` (plan's Unicode is buggy on AZERTY — ⌘Y redo instead of ⌘Z undo). macOS paste = `Key::Other(9)` + `Key::Meta` (plan/spec said `Key::Unicode('v')` + `Key::Command` — WRONG; no `Key::Command` variant exists).
4. **`.plugin(tauri_nspanel::init())` is required** (plan's Task 7 omitted it).
5. **Tauri 2 has NO `bundle.macOS.infoPlist`** → use `src-tauri/Info.plist` file (plan's JSON field doesn't exist).
6. **`NSURL::path()` returns `Option<Retained<NSString>>`** (plan's `url.path().to_string()` was wrong — handle the Option).
7. **edition 2024 → `unsafe extern "C"`** (plain `extern` is a hard error; RFC 3495).
8. **paths.rs was Windows-only** → Phase 1 CI's `cargo test --lib` failed on macOS/Linux (4 paths tests); gated to Windows until Task 5 made it cross-platform. Lesson: any platform-coupling boundary left Windows-only breaks the `cargo test --lib` CI gate on other OSes.
