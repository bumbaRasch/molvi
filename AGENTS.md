...
- **Resolved (`src-tauri/Cargo.lock`):**
  ...
  - **ASR = TWO crates: `transcribe-rs` 0.3.11 (GigaAM) + `parakeet-rs` 0.3.7 (Nemotron).** `ort` resolves to **2.0.0-rc.13** (transcribe-rs wants rc.12, parakeet-rs wants rc.13; Cargo picks the higher). ort emits tracing directly → see Logs.
  ...
  - hf-hub 1.0.0, tokio 1.53.1, cpal 0.18.1, rtrb 0.3.4, rubato 4.0.0 (`Fft` + `audioadapter` — `FftFixedIn` was REMOVED in 4.0), arboard 3.6.1, enigo 0.6.1 (**there is NO 0.7**), rusqlite 0.40.1 (`bundled`), ureq 3.3.0, regex **1.13.1** (→ regex-syntax 0.8.11).
...
- Paste uses `Key::Other(0x56)` (VK_V), NOT `Key::Unicode('v')` — enigo's Unicode path types the literal char (doesn't combine with held Ctrl). Command-mode chords use the same `Key::Other(<vk>)` shape.
