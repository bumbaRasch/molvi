...
/// Detection turns a mystifying failure into an explainable one.
/// Off-macOS → always false (the API doesn't exist).
#[cfg(target_os = "macos")]
...
pub struct AppState {
    ...
    /// Mirrors `cmd_tx`: `None` in the AppState literal, set to `Some(tx)` in
    /// `setup` once the channel is created. The `onboarding_select_model` IPC
    /// sends `(model_id, language)` here; the bg thread loops on the receiver
    /// (first-run waits for an onboarding pick, returning-run is auto-fed from
    /// `settings` in `setup`). Privacy §10.1: carries only a model id + a
    /// locale code, no user content.
    pub model_selection_tx: Mutex<Option<std::sync::mpsc::Sender<(String, String)>>>,
...
pub fn run() {
    ...
    let builder = tauri::Builder::default()
        ...
        .manage(AppState {
            ...
            model_selection_tx: Mutex::new(None),
            dictionary,
            snippets,
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            crate::ipc::onboarding_select_model,
        ...
        .setup(|app| {
            ...
            // `mut`: the bg thread applies the onboarding-chosen model + language
            // before engine spawn (load_engine dispatches on settings.model).
            let mut settings = app.state::<AppState>().settings.lock().unwrap().clone();

            // Model-choice channel (onboarding model selection). Created here; the
            // Sender is stored into AppState (mirrors `cmd_tx`: init None in the
            // literal, set to Some here). The bg thread owns the Receiver and
            // loops on it — first-run waits for `onboarding_select_model`;
            // returning-run is auto-fed below. Privacy §10.1: carries only a
            // model id + a locale code, no user content.
            let (model_tx, model_rx) = std::sync::mpsc::channel::<(String, String)>();
            *app.state::<AppState>().model_selection_tx.lock().unwrap() = Some(model_tx);
            // Returning-user auto-proceed: feed the persisted model + language so
            // the bg thread proceeds without waiting for onboarding. mpsc buffers
            // → order-independent with the thread's first recv(). First-run
            // (!onboarded) does NOT send — onboarding picks → onboarding_select_model.
            if settings.onboarded {
                let _ = app
                    ...
                    .as_ref()
                    .and_then(|tx| {
                        tx.send((settings.model.clone(), settings.language.clone()))
                            .ok()
                    });
            }

            // Launch gate — which window (if any) to surface on cold start:
            //   • First run (onboarded=false) → onboarding (the bg thread still
            //     loads engine/hotkey; onboarding observes via engine-ready).
            //   • Restart triggered from the settings window (restart_app writes
            //     a `.reopen-settings` marker — app.restart() takes no args, per
            //     Tauri 2 docs) → settings. Marker is one-shot (removed here).
            //   • Manual launch (double-click shortcut / Start menu) on an
            //     already-onboarded install → settings, so the user sees the app
            //     started instead of a silent jump-to-tray.
            //   • Autostart at login (--autostarted, passed by the autostart
            //     plugin) → nothing; you don't want a window on every login.
            // The single-instance plugin separately surfaces settings on a 2nd
            // launch while the app is already running.
            let autostarted = std::env::args().any(|a| a == "--autostarted");
            let marker = crate::paths::reopen_settings_marker();
            let restart = marker.as_ref().map(|m| m.exists()).unwrap_or(false);
                ...
                && let Some(w) = app.get_webview_window("onboarding")
            {
                let _ = w.show();
                let _ = w.set_focus();
            } else if restart && let Some(w) = app.get_webview_window("settings") {
                if let Ok(m) = &marker {
                ...
                && !autostarted
                && let Some(w) = app.get_webview_window("settings")
            {
                let _ = w.show();
                let _ = w.set_focus();
            }
            ...
            std::thread::Builder::new()
                ...
                .spawn(move || {
                    // Model download (onboarding model selection). The bg thread
                    // LOOPS on the choice channel: first-run waits for an onboarding
                    // pick (onboarding_select_model IPC); returning-run is auto-fed
                    // from settings in setup. Each pick downloads via an abortable
                    // async task whose JoinHandle reuses the existing
                    // AppState.model_download (so cancel_model_download works
                    // unchanged); on cancel/error the loop waits for the next pick
                    // (retry / choose another). Privacy §10.1: the channel carries
                    // only a model id + a locale code, no user content.
                    let (model_dir, chosen_model, chosen_language) = loop {
                        let (model_id, language) = match model_rx.recv() {
                            Ok(v) => v,
                            Err(_) => {
                        ...
                        // Disk-space pre-check (fail fast instead of running 2.4GB
                        // into ENOSPC). unwrap_or(true) keeps a statvfs/disk-query
                        // error from blocking a download that might still succeed.
                        ...
                        // Download as an abortable async task; store its JoinHandle
                        // in the existing AppState.model_download so the existing
                        // cancel_model_download (take + abort) cancels it too. Mirrors
                        // ipc::download_model's spawn verbatim (same ensure_model +
                        // Progress::new(ModelProgressEmitter::new(...)) shape). On
                        // abort the task dies before its emit → cancel is silent (no
                        // spurious "failed"); a real error emits model-download-failed.
                        ...
                        let handle = tauri::async_runtime::spawn(async move {
                            let result = model_store::ensure_model(&id_for_task, |offset| {
                                Some(hf_hub::progress::Progress::new(
                                    model_store::ModelProgressEmitter::new(
                                        app_for_task.clone(),
                                        &id_for_task,
                                        total,
                                        offset,
                            ...
                            match &result {
                                Ok(_) => {
                                    let _ =
                                        app_for_task.emit("model-download-complete", &id_for_task);
                                }
                                Err(e) => {
                                    tracing::warn!("model download failed: {e}"); // metadata-only
                                    let _ =
                                        app_for_task.emit("model-download-failed", &id_for_task);
                                }
                            }
                        ...
                        *app_handle
                        ...
                        match res_rx.recv() {
                            // success → proceed to engine spawn
                            Ok(Ok(d)) => break (d, model_id, language),
                            // failed (model-download-failed already emitted) → retry
                            Ok(Err(_)) => continue,
                            // cancelled (task aborted before its emit) → loop back
                            Err(_) => continue,
                        }
                    };
                    // load_engine dispatches on settings.model + NemotronEngine
                    // reads settings.language; the captured `settings` predates the
                    // onboarding pick, so apply the chosen values before engine spawn
                    // (first-run correctness; returning-run values already match).
                    ...
                    *app_handle.state::<AppState>().cmd_tx.lock().unwrap() = Some(cmd_tx.clone());
                    ...
                    // suppressed. Mid-session polling + a user toast are a
                    // follow-up (need Mac UX testing); this catches the
                    // held-at-startup case.
                    #[cfg(target_os = "macos")]
