...
/// Basename of `s` after the last `\` or `/`, UPPERCASED. Shared by every
/// `foreground_exe` arm so the separator set + casing contract stays in sync
/// (resolve() matches on the UPPERCASED basename — spec §6.4).
fn basename_upper(s: &str) -> String {
    s.rsplit(['\\', '/'])
        .next()
        .unwrap_or(s)
        .to_ascii_uppercase()
...
pub fn foreground_exe() -> Result<String> {
    ...
    // Basename UPPERCASED per spec (case-insensitive FS, canonical form).
    let base = basename_upper(&full);
    if base.is_empty() {
...
pub fn foreground_exe() -> Result<String> {
    ...
    let base = basename_upper(&exe.to_string_lossy());
    if base.is_empty() {
