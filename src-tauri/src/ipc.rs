...
/// Complete onboarding (Task 10 D7). Set `settings.onboarded = true`, persist
/// (mirror `set_settings`'s save path), hide the onboarding window. ALSO clean
/// exit any live practice session: send `Command::Cancel` (mirrors
/// `cancel_operation`), reset `onboarding_practice` + `mic_preview` (belt-and-
/// suspenders — the frontend cleans up too, but Done/Skip can fire from any
/// step). Registered alongside the other AppState-mutating commands in lib.rs.
#[tauri::command]
pub fn complete_onboarding(app: AppHandle, state: State<'_, AppState>) -> Result<(), MolviError> {
    {
        let mut g = state.settings.lock().unwrap();
        g.onboarded = true;
        if let Err(e) = g.save() {
    ...
    // Processing; a no-op in Idle). Drops the live session cleanly so a stale
    // finalize doesn't emit a practice-result after the window hid.
    if let Some(tx) = state.cmd_tx.lock().unwrap().as_ref() {
