...
/// Marker file written by `restart_app` (a settings-triggered restart) so the
/// restarted process reopens the settings window. `app.restart()` (relaunch)
/// takes no args (verified vs Tauri 2 docs), so a side-channel file is the
/// signal; cleared by `setup` after it surfaces the window.
pub fn reopen_settings_marker() -> Result<PathBuf> {
    Ok(app_data_dir()?.join(".reopen-settings"))
}
