...
// Self-labeled controls (Toggle/Select/TextInput/SecretInput/Slider/Textarea)
// render their own <label>; drop them straight into a SettingsGroup.
...
export function SecretInput(label: string, value: string, onChange: (v: string) => void): TextCtrl {
  // ponytail: SecretInput = TextInput with type=password. Show/hide eye-toggle
  // skipped (would need a button + state wiring beyond the 5-line budget). Task 15
  // can add the eye if the API-key field wants reveal. Upgrade path: wrap result
  // and append a <button> that flips input.type between "password"/"text".
  return TextInput(label, value, onChange, { type: "password" });
}

interface AreaCtrl { wrap: HTMLElement; set: (v: string) => void; get: () => string; }

export function Textarea(label: string, value: string, onChange: (v: string) => void): AreaCtrl {
