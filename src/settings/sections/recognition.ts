...
function fmtBytes(bytes: number): string {
  ...
  return new Intl.NumberFormat(getCurrentLang(), { style: "unit", unit, maximumFractionDigits: 1 }).format(bytes);
}
...
export const buildRecognition: SectionBuilder = (store) => {
  ...
  // ── Model picker (2 cards: GigaAM / Nemotron) ───────────────────────────
  const MODELS = [
  ...
  function on<T>(event: string, cb: (payload: T) => void): void {
    void (async () => {
      ...
      } catch (e) { console.error(`${event} listen failed`, e); } // metadata-only
    })();
  }
  on<{ model: string; bytes: number; total: number; pct: number }>("model-download-progress", (p) => {
    downloading = { id: p.model, pct: p.pct, bytes: p.bytes, total: p.total };
    renderPicker();
  });
  on<{ model: string }>("model-download-complete", () => {
    downloading = undefined;
    toast("success", t("models.download_complete"));
    refreshStatuses(); // re-fetch cached flags → card becomes ready/active
  });
  on<{ model: string }>("model-download-failed", () => {
    downloading = undefined;
  ...
  return {
    el: root,
    cleanup: () => { disposed = true; unlistens.forEach((u) => u()); },
  };
};
