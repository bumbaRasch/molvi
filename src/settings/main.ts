...
import type { Section, Settings, State } from "./types";
...
// One list drives both the sidebar (id+icon) and the section builder — no
// parallel id map to drift out of sync.
const SECTIONS = [
  { id: "recognition", icon: ICONS.recognition, build: buildRecognition },
  { id: "microphone",  icon: ICONS.microphone,  build: buildMicrophone  },
  { id: "text",        icon: ICONS.text,        build: buildText        },
  { id: "dictionary",  icon: ICONS.dictionary,  build: buildDictionary  },
  { id: "snippets",    icon: ICONS.snippets,    build: buildSnippets    },
  { id: "history",     icon: ICONS.history,     build: buildHistory     },
  { id: "hotkey",      icon: ICONS.hotkey,      build: buildHotkey      },
  { id: "overlay",     icon: ICONS.overlay,     build: buildOverlay     },
  { id: "updates",     icon: ICONS.updates,     build: buildUpdates     },
  { id: "about",       icon: ICONS.about,       build: buildAbout       },
] as const;
...
export function selectSection(id: string): void {
  ...
  const builder = SECTIONS.find((s) => s.id === id)?.build;
